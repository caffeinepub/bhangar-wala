...
import { ArrowLeft, QrCode, Banknote, Copy, Check, Loader2 } from 'lucide-react';
...
export default function Payment() {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      await updateStatus.mutateAsync({ bookingId, status: BookingStatus.completed });
      navigate({ to: '/payment-success', search: { bookingId, amount: totalAmount } });
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <div className="flex-1 px-4 py-5 space-y-4 pb-32">
        ...
        {paymentMethod === PaymentMethod.upi && (
          <div className="bg-card rounded-2xl border border-border p-4 space-y-3">
            <p className="font-heading font-bold text-sm text-foreground">UPI Payment</p>
            ...
            <div className="flex items-center gap-2 bg-muted rounded-xl px-3 py-2">
              <span className="flex-1 text-sm font-medium text-foreground">bhangarwala@upi</span>
              <button onClick={handleCopyUpi} className="text-primary">
                {upiCopied ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
              </button>
            </div>
            <p className="text-xs text-muted-foreground text-center">
              Scan the QR code or use the UPI ID above to pay
            </p>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-background border-t border-border px-4 py-4">
        <Button
          ...
          onClick={handlePay}
          disabled={createPayment.isPending || updateStatus.isPending || totalAmount <= 0}
        >
